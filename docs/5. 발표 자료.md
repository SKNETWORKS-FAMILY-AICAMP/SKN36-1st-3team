...
<div class="cover-team">SKN36 · 1st Project · TEAM 3</div>
...
|:---:|:---:|:---:|
| ![w:400](./images/policy-education.png) | ![w:400](./images/policy-return.png) | ![w:400](./images/policy-device.png) |
| **운전면허 관리와 교육** | **운전면허 자진반납 지원** | **차량 안전장치 지원** |
...
# SAFE
![w:500](./images/policy-device.png)
...
## 시계열 예측 모델 후보 및 평가 지표

| 지표 | 의미 | 해석 |
|---|---|---|
| ARIMA | 과거 값과 변화 패턴을 이용해 예측 | 데이터 흐름이 안정적일수록 좋음 |
| SARIMA | ARIMA에 월·계절처럼 반복되는 주기를 반영 | 계절성이 있는 데이터에 적합 |
| Prophet | 장기 추세와 반복 패턴을 학습해 예측 | 장기 예측 시 시각화가 직관적 |

| 지표 | 의미 | 해석 |
|---|---|---|
| MAE | 실제값과 예측값 차이의 평균 | 작을수록 좋음 |
| RMSE | 큰 예측 오차에 더 큰 패널티 부여 | 작을수록 좋음 |
| MAPE | 실제값 대비 평균 오차율 | 일반 사용자가 이해하기 쉬움 |

그래프 모양만 보고 모델을 고르지 않고, 위 지표들을 함께 비교해 검토했습니다.
...
| 테이블 별 조인 미실시 | 중복 내용 조인 해서 ERD 만들겠음 |
